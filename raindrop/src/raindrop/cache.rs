...
impl FileItemCache {
    pub fn default() -> FileItemCache {
        return FileItemCache::new(&*tilde(&format!("~/.local/state/anyrun/raindrop.json",)));
    }
...
impl FileItemScoreCache {
    pub fn default() -> Self {
        return Self::new(&*tilde(&format!(
            "~/.local/state/anyrun/raindrop.score.json",
        )));
    }
    ...
    pub fn update_item_scores(&self) {
        ...
        let access_log_file =
            tilde(&format!("~/.local/state/anyrun/raindrop.access_log")).to_string();
